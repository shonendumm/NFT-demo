...
# change the values here depending on the response returned from upload_IPFS() / create_metadata.py
# dog_metadata_dic = {
    ...
    for token_id in range(number_of_collectibles):
        ...
        #     set_tokenURI(token_id, advanced_collectible, dog_metadata_dic[breed])
            with open("./metadata/tokens_metadata_url.txt", "r") as file:
                ...
                print(f"Setting tokenURI of {token_id}")
                print(lines[token_id])
                set_tokenURI(token_id, advanced_collectible, lines[token_id])
